...
[![Proofs](https://img.shields.io/badge/proofs-32%20proved%20%2F%200%20sorry-brightgreen)](Kuramoto)
...
The full library is **32 theorem statements, all machine-checked `sorry`-free, `admit`-free, with no new axioms**: 14 in the algebraic core, 15 on the ODE / convergence frontier, and 3 in the discrete fleet-clock module.
...
**Honest scope.** This is **confinement** (the skew never grows past its starting spread), *not* convergence to a single phase. Exact synchrony `diameter → 0` in discrete time is named future work: it needs a changing-argmax strict-contraction argument plus a discrete Barbalat summability lemma. The load-bearing new content versus the continuous theorem is the step bound itself, since the continuous proof gives no rate.
...
### Unreleased (2026-07-11) — discrete fleet-clock

Adds `Kuramoto/Discrete.lean`, the discrete-time extension (not yet in the Zenodo paper; paper stays at 29). Repo now carries **32** machine-checked theorem statements, `sorry`-free, `admit`-free, footprint `{propext, Classical.choice, Quot.sound}`.
...
- Honest scope: bounded skew (confinement), not exact synchrony; discrete `diameter → 0` is named future work.
